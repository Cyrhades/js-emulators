...
export class CPU6502 {
  ...
  public step(onTick?: (cycles: number) => void): number {
    if (this.clockMode !== ClockMode.running && this.clockMode !== ClockMode.waitForInterrupt) {
      return 0;
    }
    ...
    if (this.interruptPending) {
      ...
      } else if (this.interruptPending === "NMIB") {
        this.execInterruptNMIB();
      } else if (this.interruptPending === "RESB") {
        this.execInterruptReset();
      }
      if (onTick) onTick(7);
    ...
    if (instruction === undefined) {
      // Fallback for NOP or unhandled opcodes
    ...
    if (instruction.dataBytes === 1) {
      ...
      this.programCounter++;
    } else if (instruction.dataBytes === 2) {
      instructionParam = this._read16BitValue(this.programCounter);
      this.programCounter += 2;
    }

    // Basic cycle estimation by instruction dataBytes & type
    let cycles = 2;
    if (instruction.dataBytes === 1) cycles = 3;
    if (instruction.dataBytes === 2) cycles = 4;
    if (instructionOpCode === 0x20 || instructionOpCode === 0x60 || instructionOpCode === 0x40) cycles = 6; // JSR, RTS, RTI
    if (instructionOpCode === 0x48 || instructionOpCode === 0x08) cycles = 3; // PHA, PHP
    if (instructionOpCode === 0x68 || instructionOpCode === 0x28) cycles = 4; // PLA, PLP

    const preCycles = Math.max(1, cycles - 1);
    const postCycles = cycles - preCycles;

    if (onTick) onTick(preCycles);
    instruction.func(this, instructionParam ?? 0);
    this.instructionsExecuted++;
    if (onTick && postCycles > 0) onTick(postCycles);
  ...
  public _convertValueTo8BitAndSetStatusFlags(
    ...
    if (checkCarry) {
      const carry = (value & 0b100000000) === 0b100000000;
      if (carry) {
