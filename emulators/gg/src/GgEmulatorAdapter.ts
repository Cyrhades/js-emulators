...
import JSSMS from "../../sms/src/jssms/jssms-core.js";
import { decodeSmsCheatCode } from "../../sms/src/smsCheatDecoder";

const SCREEN_WIDTH = 160;
const SCREEN_HEIGHT = 144;
const GG_X_OFFSET = 48;
const GG_Y_OFFSET = 24;
const sharedMemoryStorage = new Map<string, string>();

export class GgEmulatorAdapter implements Emulator {
  public readonly id = "gg";
  public readonly name = "Sega Game Gear";
    ...
    this.gameGenie = {
      ...
      addGameGenieCode: (code: string, description: string) => {
        ...
        const newCode: CheatCode = {
          id: `gg_gg_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
          code,
          description,
          active: true,
          decoded,
  ...
  private loadCodesFromStorage(): void {
    ...
    try {
      ...
      console.warn("Could not load Game Gear Game Genie codes from storage", e);
    }
  }
  ...
  private saveCodesToStorage(): void {
    ...
    try {
      ...
      console.warn("Could not save Game Gear Game Genie codes to storage", e);
    }
  }
  ...
  public async loadRom(data: Uint8Array): Promise<void> {
    ...
    if (!success) {
      throw new Error("Unable to load Game Gear ROM.");
    }
  ...
  private runSingleFrame(): void {
    ...
    // Crop 160x144 Game Gear viewport from VDP 256x192 display buffer
    const display = this.sms.vdp.display;
    if (display) {
      let destIdx = 0;
      for (let y = 0; y < SCREEN_HEIGHT; y++) {
        let srcIdx = ((y + GG_Y_OFFSET) * 256 + GG_X_OFFSET) * 4;
        for (let x = 0; x < SCREEN_WIDTH; x++) {
          this.videoBuffer[destIdx] = display[srcIdx];
          this.videoBuffer[destIdx + 1] = display[srcIdx + 1];
          this.videoBuffer[destIdx + 2] = display[srcIdx + 2];
          this.videoBuffer[destIdx + 3] = 255;
          destIdx += 4;
          srcIdx += 4;
        }
      }
  ...
  public handleInput(input: EmulatorInput): void {
    ...
    if (b.start || b.pause) {
      this.sms.keyboard.ggstart = 0x7f;
    } else {
      this.sms.keyboard.ggstart = 0xff;
    }
  }
}
